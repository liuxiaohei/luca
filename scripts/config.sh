...
function bind_dir_path(){
    BIND_DIR=${BIND_DIR_ROOT}/${APPLICATION_NAME}-${PORT}
}

#get container name
function container_name(){
    CONTAINER_NAME=${APPLICATION_NAME}-${PORT}
}
...
function docker_run(){
    ...
    -v ${BIND_DIR}/config:/data/config \
    -v ${BIND_DIR}/tmp:/data/tmp \
    --env PORT=${PORT} \
...
function mvn_package(){
    ...
    echo $(pwd)
    ORG_PATH=target/${APPLICATION_NAME}-server.jar
    echo ${ORG_PATH}
